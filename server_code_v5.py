...
def api_generate_ai_summary(pid):
    ...
    # STRICT SHELL SANITIZATION
    clean_context = re.sub(r'[^a-zA-Z0-9 \.,:\-\(\)\%]', ' ', context)
    
    prompt = f"Instruction: Summarize this patient concisely in 3 short sentences.\n\nData: {clean_context}\n\nSummary:"

    # FIX 1: Increased -n to 400 so the model has enough room to "think" and write the answer.
    cmd = (
        "cd /data/home/qnxuser/llama.cpp/ && "
        "export LD_LIBRARY_PATH=`pwd`/bin:$LD_LIBRARY_PATH && "
        f"echo \"/exit\" | bin/llama-cli -m models/qwen3-751.63M-Q4_K_M.gguf -n 400 --log-disable "
        f"-p \"{prompt}\" 2>/dev/null"
    )
    ...
    try:
        ...
        # Remove the prompt echo so we are left with only the model's generation
        if prompt in output:
            output = output.replace(prompt, "")
            
        # FIX 2: Filter out the reasoning blocks ([Start thinking]...[End thinking] or <think>...</think>)
        output = re.sub(r'\[Start thinking\].*?\[End thinking\]', '', output, flags=re.DOTALL | re.IGNORECASE)
        output = re.sub(r'<think>.*?</think>', '', output, flags=re.DOTALL | re.IGNORECASE)
        
        # In case the model gets cut off and doesn't print [End thinking], just remove the opening tag
        output = output.replace("[Start thinking]", "").replace("[End thinking]", "").strip()
        
        # Clean up lingering cli artifacts like terminal metrics or the exit command
        output = re.sub(r'\[Prompt:.*?\]', '', output)
        output = output.replace("> /exit", "").replace(">", "").strip()

        if not output:
            output = "AI inference produced no final output. (Model may have run out of tokens while thinking)."
    ...
    <div class="card shadow-sm border-primary mb-4">
        <div class="card-body bg-light">
            <div class="d-flex justify-content-between align-items-center mb-2">
                <h5 class="text-primary m-0">Llama.cpp Clinical Summary</h5>
            ...
            <div id="ai-box" class="p-3 border rounded bg-white mt-3" style="min-height: 70px; font-family: monospace; white-space: pre-wrap;">
                <span class="text-muted">Click the button to invoke llama.cpp locally on this server...</span>
            </div>
        </div>
    </div>
...
function requestAiSummary() {
    ...
    box.innerHTML = '<div class="d-flex align-items-center text-primary"><div class="spinner-border spinner-border-sm me-2" role="status"></div><span>Generating AI summary using llama.cpp... Please wait (this can take up to 30-45 seconds).</span></div>';
