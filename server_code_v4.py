...
def api_generate_ai_summary(pid):
    ...
    # THE FIX: echo "/exit" | ... prevents the infinite ">" loop by feeding the exit command directly.
    cmd = (
        "cd /data/home/qnxuser/llama.cpp/ && "
        "export LD_LIBRARY_PATH=`pwd`/bin:$LD_LIBRARY_PATH && "
        f"echo \"/exit\" | bin/llama-cli -m models/qwen3-751.63M-Q4_K_M.gguf -n 120 --log-disable "
        f"-p \"{prompt}\" 2>/dev/null"
    )

    try:
        # Removed DEVNULL stdin to allow the piped "/exit" string through.
        ...
        # Clean up output to isolate just the AI's summary
        if "Summary:" in output:
            output = output.split("Summary:")[-1].strip()
            
        # Remove any lingering chat prompt arrows or the echoed exit command
        output = output.replace("> /exit", "").replace(">", "").strip()

        if not output:
            output = "AI inference produced no output. (Check model parameters)"
...
function requestAiSummary() {
    ...
    box.innerHTML = '<div class="d-flex align-items-center text-primary"><div class="spinner-border spinner-border-sm me-2" role="status"></div><span>Generating AI summary using llama.cpp... Please wait (this can take up to 20-30 seconds).</span></div>';
