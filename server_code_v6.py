...
def api_generate_ai_summary(pid):
    ...
    # STRICT SHELL SANITIZATION: Removes tricky characters that hang the shell
    clean_context = re.sub(r'[^a-zA-Z0-9 \.,:\-\(\)\%]', ' ', context)
    
    prompt = f"Summarize this patient concisely in 3 short sentences.\nData: {clean_context}\n\nSummary:"

    # FIXED COMMAND: Pipes /exit into stdin to close the interactive prompt safely. 
    # Points to TinyLlama instead of Qwen.
    cmd = (
        "cd /data/home/qnxuser/llama.cpp/ && "
        "export LD_LIBRARY_PATH=`pwd`/bin:$LD_LIBRARY_PATH && "
        f"echo \"/exit\" | bin/llama-cli -m models/tinyllama-1.1b-chat-v1.0.Q4_K_M.gguf -n 300 --log-disable "
        f"-p \"{prompt}\" 2>/dev/null"
    )

    try:
        # Run command with 120s timeout (TinyLlama is fast)
        ...
        # Isolate just the AI's generation (strip the prompt echo)
        ...
        if not output:
            output = "AI inference produced no output. (Model returned blank)."
